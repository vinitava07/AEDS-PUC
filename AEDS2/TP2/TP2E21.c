...
void heapfy(Lista *l, int i)
{
    ...
    for (int j = i; j > 0; j = (j - 1) / 2)
    {
        ...
        if (l->pL[j]->altura > l->pL[(j - 1) / 2]->altura)
        {
            aux = l->pL[j];
            l->pL[j] = l->pL[(j - 1) / 2];
            l->pL[(j - 1) / 2] = aux;
...
void reheapfy(Lista *l, int tam)
{
    ...
    {
        if (l->pL[i * 2 + 1]->altura > l->pL[i * 2 + 2]->altura)
        {
            filho = i * 2 + 1;
        }
        else
        ...
        if (l->pL[filho]->altura > l->pL[i]->altura)
        {
            aux = l->pL[i];
            l->pL[i] = l->pL[filho];
            l->pL[filho] = aux;
...
void heapParcialSort(Lista *l)
{
    ...
    for (int i = k+1; i <= l->cont - 1; i++)
    {
        if (l->pL[i]->altura < l->pL[0]->altura)
        {
            aux = l->pL[i];
            l->pL[i] = l->pL[0];
            l->pL[0] = aux;
...
void imprimeLista(Lista *l)
{
    ...
    // for (int i = 0; i < l->cont / 2 - 1; i++)
    // {
    //     printf("pai: %d  ", l->pL[i]->altura);
    //     printf("filho: %d  ", l->pL[i * 2 + 1]->altura);
    //     printf("filho: %d  \n", l->pL[i * 2 + 2]->altura);
    // }
}
...
int main()
{
    ...
    FILE *fw = fopen("784759_HeapSortParcial.txt", "w");
    inc = time(NULL);
    ...
    for (int i = 0; i < L.cont; i++)
    {
        ...
        {
            if (L.pL[i]->altura == L.pL[j]->altura)
            {
