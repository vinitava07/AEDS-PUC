...
class Lista {
    ...
    public void quickSort(int esq, int dir) {
        ...
        int i = esq;
        int j = dir;
        Personagem pivo = this.pL[(esq + dir) / 2].clone();
        Personagem aux;

        while (i <= j) {
            while (this.pL[i].getCorDoCabelo().compareTo(pivo.getCorDoCabelo()) < 0 && i < dir) {
                i++;
            }
            while (this.pL[j].getCorDoCabelo().compareTo(pivo.getCorDoCabelo()) > 0 && j > esq) {
                j--;
            }
            if (i <= j) {
                aux = this.pL[i].clone();
...
public class TP2E22 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        Lista l = new Lista();
        FileWriter fw = new FileWriter("Vinicius_784759_quickParcial.txt");
        ...
        l.quickSort(0, l.cont-1);
        Personagem aux;
    for (int i = 0; i < l.cont; i++)
    {
        for (int j = i; j < l.cont; j++)
        {
            if (l.pL[i].getCorDoCabelo().compareTo(l.pL[j].getCorDoCabelo()) == 0)
            {

                if (l.pL[i].getNome().compareTo(l.pL[j].getNome())> 0)
                {
                    aux = l.pL[i].clone();
                    l.pL[i] = l.pL[j].clone();
                    l.pL[j] = aux.clone();
